...
const ServiceItem = ({service, barbershop, isAuthenticated}: serviceItemProps) => {
  ...
  const [dayBookings, setDayBoogkings] = useState<Booking[]>([])
            ...
            <div className="flex items-center justify-between mt-3">
              ...
              <Sheet open={sheetIsOpen} onOpenChange={setSheetIsOpen}>
                ...
                <SheetContent className="p-0">
                  ...
                  {date && (
                    <div className="flex gap-3 overflow-auto py-6 px-5 border-y border-solid border-secundary [&::-webkit-scrollbar]:hidden">
                      {timeList.map((time) => (
                        <Button variant={hour === time ? 'default' : 'outline'} className="rounded-full" key={time} onClick={() => handleHourClick(time)}>
                          {time}
                        </Button>
                      ))}
                    </div>
